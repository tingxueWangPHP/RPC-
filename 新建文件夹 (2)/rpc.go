...
func DialServer(protocol, address string) *Client {
	return &Client{
		protocol: protocol,
		address:  address,
	}
...
func (c *Client) Call(method string, timeout time.Duration, ret interface{}, args ...interface{}) error {
	if reflect.TypeOf(ret).Kind() != reflect.Ptr {
		return errors.New("type error")
	}

	conn, err := net.DialTimeout(c.protocol, c.address, timeout)
	if err != nil {
		return err
	}
